...
@Bean(name = "denyMeetModule", parent = MeetComponent.class, active = true)
public class DenyMeetModule
		extends AbstractModule {

	private static final Criteria CRITERIA = ElementCriteria.name("iq").add(ElementCriteria.name("deny", "tigase:meet:0"));
	...
	public CompletableFuture<Packet> processPacket(Packet packet) throws ComponentException, TigaseStringprepException {
		...
		Element allowElem = Optional.ofNullable(packet.getElemChild("deny", "tigase:meet:0"))
				.orElseThrow(() -> new ComponentException(Authorization.BAD_REQUEST, "Missing `allow` element"));

		List<BareJID> denied = Optional.ofNullable(allowElem.mapChildren(el -> "participant".equals(el.getName()),
																		  el -> BareJID.bareJIDInstanceNS(
																				  el.getCData())))
				.orElse(Collections.emptyList());

		for (BareJID jid : denied) {
			meet.deny(jid);
		}
